...
\todo[inline]{Does this sort of conjecture belong in a more succinct paper?}
...
\todo[inline]{This section is confusing at present as all MITRE could be removed if deemed surplus
	to requirements}
...
\begin{itemize}
	\item It helps in understanding the underlying patterns and themes in vulnerability descriptions that contribute to specific CVSS ratings.
	      \todo[inline]{This has not been mentioned previously and is another reason why this
		      technique could be useful, could be ran on CVE descriptions on entry to MITRE}
...
\begin{itemize}
	...
	\item Latent Dirichlet Allocation (LDA): We then employ a more sophisticated topic modeling technique, LDA, in conjunction with Word2Vec embeddings. This allows us to discover latent themes in the vulnerability descriptions and how they relate to CVSS metrics.
	      ...
	      \todo[inline]{Add in topic merging to here and replace purity analysis as that is just part of
		      the LDA process}
...
\todo[inline]{This part needs to also contain some introduction to the next K-Means section, the LDA
	results explanation could still be part of this or used as a transition after the k-means bit.}
...
\todo[inline]{As we will be providing the "artifacts", git repo I assume, we should link to it for
	these sections and the provided code, though might require some cleanup for it to be usable for
	anyone else...}
...
\todo[inline]{The choice of five here is weak, unsure what to do about that though, there may be
	some stats reason to choose a specific number here, some kind of elbow method on distribution
	etc}
...
\todo[inline]{Lots of talk here about the topics or themes not being known, however to some degree
	we do know or have some inkling based on the CWE ( assuming that there is a CWE for that
	description)}
...
\begin{itemize}
	\item Custom stopwords are defined, combining standard English stopwords with domain-specific terms (e.g., \say{vulnerability}, \say{attacker}).
	      \todo[inline]{Might be good to explain how they are tokenized?}
...
\todo[inline]{Need to add small specific stop word example to show why they need removing, could
	also show a distribution with them still there to show the problem}
...
\todo[inline]{Need an introducing sentence before this statement about hyperparameters, this
	sentence feels like its too defensive}
...
\begin{itemize}
	...
	\item $\alpha$: {\say{symmetric}}
	      \begin{itemize}
		      ...
		      \item The \say{symmetric} setting assumes all topics are equally likely \textit{a
...
\todo[inline]{Maybe this doesn't need to be its own subsubsection}
